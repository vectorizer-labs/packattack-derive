...
use ansi_term::Colour::{Purple, Green, Red};
...
use attributes::{ ParentDataType, enums::get_enum_attributes };
...
pub fn from_bits(input: TokenStream) -> TokenStream 
{
    ...
    {
        Ok(foo) => foo,
        Err(e) => panic!("PackAttack : Failed to parse Abstract Syntax Tree.")
    };
...
{
    println!("{}", Purple.paint("PARSING AST :     "));
    let ast : syn::DeriveInput = match syn::parse(input)
    {
        Ok(res) => { print!("{}", Green.paint("OK")); res },
        Err(e) => { panic!("{} : {}", Red.paint("FAILED"), e); }
    };
...
pub fn from_bytes(input: TokenStream) -> TokenStream 
{
    ...
    //TODO: fix to #name + _size
    let const_name = quote!{ blah };
